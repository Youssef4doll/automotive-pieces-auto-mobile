...
try {
  ...
  await page.getByLabel('Nom et prénom').fill('Compte Essai');
  await page.getByLabel('Téléphone').fill('20 333 444');
  await tapLabel(page, 'Gouvernorat');
  await tap(page, 'Ariana');
  await page.getByLabel('Adresse').fill('5 rue du Compte, Ariana');
  ...
  await page.waitForTimeout(800);
  await page.getByLabel('Nom et prénom').fill('nom@exemple.tn');
...
try {
  ...
  await page.getByLabel('Adresse e-mail').fill(email);
  await page.getByLabel('Mot de passe').fill('mauvais-mot');
